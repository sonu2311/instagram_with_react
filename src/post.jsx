...
function MainPostContent({post, post_delete}){
    ...
    <>
      <div style={{boxSizing: 'border-box'}}>
        <div className="hsplit" style={{boxSizing: 'border-box', position: 'relative'}}>
          ...
          <div>
            <a href={"#/profile/" + post.user_id} style={{textDecoration: 'none'}}>
              <div style={{color: '#333', marginTop: '6px', fontWeight: 600, fontSize: '14px'}}> 
                {post.name}
              </div>
            </a>
            <div style={{fontSize: '12px', fontWeight: 700, marginTop: '3px', color: '#666'}}>
                {post.post_time_str}
            </div>
          </div>
...
function LikeAndDislike({post, show_all_comments, num_comments}){
...
function AddNewComment({post, comment_now, comment_text, setComment_text}){
  return (
    <>
...
function AllCommentList({all_comment_list}){
  return (
    <>
...
function Post({post}) {
  ...
  return (
    <div style={{color: '#333', marginBottom: '25px', borderRadius: '4px', padding: '20px', boxShadow: '2px 2px 5px #888888', backgroundColor: '#fff'}}>
      <div style={{boxSizing: 'border-box'}}>
        <MainPostContent post={post} post_delete={post_delete} />
        <LikeAndDislike post={post} show_all_comments={show_all_comments} num_comments={num_comments}  />
        <AddNewComment post={post} comment_now={comment_now} comment_text={comment_text} setComment_text={setComment_text} />
        <hr /> 
       <AllCommentList all_comment_list={all_comment_list} />
      </div>
    </div>
  );
}
